...
class SplashActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // ✅ Add screen size logging
        val displayMetrics = resources.displayMetrics
        val width = displayMetrics.widthPixels
        val height = displayMetrics.heightPixels
        android.util.Log.d("ScreenSize", "Width: $width, Height: $height")
    ...
    private fun startSplashVideo() {
        ...
        try {
            ...
            vv.setOnPreparedListener { mp: MediaPlayer ->
                ...
                // ✅ Adaptive scaling for all screen sizes with centered pivot
                val displayMetrics = resources.displayMetrics
                val screenWidth = displayMetrics.widthPixels.toFloat()
                val screenHeight = displayMetrics.heightPixels.toFloat()
                
                val videoWidth = 1080f
                val videoHeight = 1920f
                
                val scaleX = screenWidth / videoWidth
                val scaleY = screenHeight / videoHeight
                val scale = maxOf(scaleX, scaleY)
                
                // ✅ Center the scaling pivot for equal cropping
                vv.pivotX = videoWidth / 2f
                vv.pivotY = videoHeight / 2f
                ...
                vv.translationX = (screenWidth - scaledWidth) / 2f
                vv.translationY = (screenHeight - scaledHeight) / 2f
