...
class MealRepository(
    ...
    suspend fun initializeSampleData() {
        ...
        // Create today's and tomorrow's meal plans
        val today = LocalDate.now().toString()
        val tomorrow = LocalDate.now().plusDays(1).toString()
        
        val existingTodayPlan = getMealPlanByDate(today)
        val existingTomorrowPlan = getMealPlanByDate(tomorrow)
        
        if (existingTodayPlan == null) {
            val todayMealPlan = MealPlan(
                date = today,
                breakfastId = 1,  // Oatmeal with Berries (local)
                lunchId = 9,      // Chicken Tikka (GitHub)
                lunch2Id = 4,     // Fruit Salad Bowl (local snack)
                dinnerId = 11     // Salmon & Quinoa (GitHub)
            )
            insertMealPlan(todayMealPlan)
        }
        
        if (existingTomorrowPlan == null) {
            val tomorrowMealPlan = MealPlan(
                date = tomorrow,
                breakfastId = 5,  // Pancake Breakfast (GitHub)
                lunchId = 10,     // Grilled Chicken Breast (GitHub)
                lunch2Id = 4,     // Fruit Salad Bowl (local snack)
                dinnerId = 12     // Shrimp Tacos (GitHub)
            )
            insertMealPlan(tomorrowMealPlan)
        }
    }
}
