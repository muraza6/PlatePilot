    ...
    suspend fun loadSevenDayCache() {
        ...
        for (dayOffset in 0..6) {
            ...
            try {
                ...
                if (mealPlan != null) {
                    val breakfast = mealPlan.breakfastId?.let { repository.getMealById(it) }
                    val lunch = mealPlan.lunchId?.let { repository.getMealById(it) }
                    val lunch2 = mealPlan.lunch2Id?.let { repository.getMealById(it) }
                    val dinner = mealPlan.dinnerId?.let { repository.getMealById(it) }
