...
class MealRepository(
    ...
    private suspend fun ensureSevenDayMealPlans() {
        ...
        val filteredMeals = if (dietaryPreference == "None" || dietaryPreference.isEmpty()) {
            ...
            allMeals.filter { meal ->
                ...
                // Handle Gluten-Free (exact match only)
        ...
        // Use null if no meals available for that category
        val breakfasts = if (breakfastMeals.isNotEmpty()) {
            (0..6).map { breakfastMeals[it % breakfastMeals.size].id }
        } else {
            listOf(null, null, null, null, null, null, null) // No meals available - show empty
        }
        ...
        val snackId = snackMeals.firstOrNull()?.id  // null if no snacks available
        
        // Create or update meal plans for the next 7 days with variety
        for (dayOffset in 0..6) {
            ...
            if (existingPlan == null) {
                ...
                // Update existing plan to reflect new dietary preference
    ...
    suspend fun syncMealsFromGitHub(githubUrl: String): Result<String> {
        return try {
            Log.d(TAG, "Starting meal sync from GitHub")
            
            // Fetch meals from GitHub
            val result = githubMealService.getMealsFromGitHub(githubUrl).firstOrNull()
            
            if (result?.isSuccess == true) {
                val githubMeals = result.getOrNull()
                if (!githubMeals.isNullOrEmpty()) {
                    // Clear existing meals and insert new ones
                    Log.d(TAG, "Clearing existing meals and inserting ${githubMeals.size} new meals")
                    mealDao.deleteAllMeals()
                    insertMeals(githubMeals)
                    
                    // Regenerate meal plans with new meals
                    ensureSevenDayMealPlans()
                    
                    Log.d(TAG, "Successfully synced ${githubMeals.size} meals from GitHub")
                    Result.success("Synced ${githubMeals.size} meals successfully")
                } else {
                    Log.w(TAG, "No meals found in GitHub response")
                    Result.failure(Exception("No meals found in GitHub repository"))
                }
            } else {
