            ...
            NavHost(
                ...
                composable("plans_meal_detail") {
                    selectedMealId?.let { mealId ->
                        // Load meal from repository using the ID
                        val plansViewModel: com.bodycalc.platepilot.ui.viewmodel.PlansViewModel = androidx.lifecycle.viewmodel.compose.viewModel()
                        val allMeals by plansViewModel.mealsByType.collectAsState()
                        
                        // Find the meal from all meal types
                        val meal = allMeals.values.flatten().find { it.id == mealId }
